...
try:
    ...
    from lwga_upscaler import csrc as _C
except ImportError:
    ...
    if cuda_toolkit_available():
        ...
        if os.path.exists(os.path.join(build_dir, "lwga_upscaler_cuda.so")) or os.path.exists(
            os.path.join(build_dir, "lwga_upscaler_cuda.lib")
        ):
            ...
            with Console().status(
                "[bold yellow]lwga_upscaler: Setting up CUDA (This may take a few minutes the first time)",
        ...
        Console().print(
            "[yellow]lwga_upscaler: No CUDA toolkit found. lwga_upscaler will be disabled.[/yellow]"
        )
